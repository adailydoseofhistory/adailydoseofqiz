...
<div class="wrap" style="padding:2rem 20px;">
  ...
  <div style="text-align:center;margin-bottom:2rem;padding-bottom:1.5rem;border-bottom:1px solid var(--border);">
    ...
    <p style="font-style:italic;color:var(--sepia);font-size:13px;margin-top:6px;">Fill in the form below — copy the generated code and upload it to GitHub</p>
  </div>
  ...
  <div style="background:var(--cream);border:1px solid var(--border);padding:2rem;">
    ...
    <div style="margin-bottom:1.5rem;">
      <label style="display:block;font-size:10px;letter-spacing:2.5px;text-transform:uppercase;color:var(--sepia);font-weight:600;margin-bottom:8px;">Image URL (optional)</label>
    ...
    <div style="margin-bottom:1.5rem;">
      <label style="display:block;font-size:10px;letter-spacing:2.5px;text-transform:uppercase;color:var(--sepia);font-weight:600;margin-bottom:8px;">Your Article (supports **bold**, ## Headings, > quotes)</label>
      <textarea id="f-body" style="width:100%;font-family:'Lora',serif;font-size:14px;color:var(--ink);background:var(--parchment);border:1px solid var(--border);padding:10px 14px;outline:none;min-height:250px;resize:vertical;line-height:1.8;" placeholder="Begin your chronicle here..."></textarea>
    </div>
  ...
  <div id="output-wrap" style="display:none;margin-top:2rem;">
    <div style="background:var(--ink);color:var(--gold);font-family:'Playfair Display',serif;font-size:16px;padding:1rem 1.5rem;margin-bottom:0;border-bottom:2px solid var(--gold);">
      ✦ Your post file is ready!
    </div>
    ...
    <div style="background:var(--cream);border:1px solid var(--border);border-left:4px solid var(--gold);padding:1.25rem 1.5rem;margin-top:1.5rem;">
      <div style="font-family:'Playfair Display',serif;font-size:16px;font-weight:700;color:var(--ink);margin-bottom:10px;">📌 How to publish this post:</div>
      <ol style="font-size:13px;color:var(--ink-light);line-height:2;padding-left:1.25rem;">
        ...
        <li>Go to your GitHub repo: <strong>adailydoseofhistory.github.io</strong></li>
        ...
        <li>Your post goes live in <strong>1–2 minutes!</strong> ✅</li>
      </ol>
    </div>
  </div>
</div>
...
function generatePost() {
  ...
  const author = document.getElementById('f-author').value.trim() || 'The Chronicler';
  const image = document.getElementById('f-image').value.trim();
  const body = document.getElementById('f-body').value.trim();

  if (!title || !body) { alert('Please fill in the title and article body.'); return; }

  const d = new Date();
  const dateStr = d.getFullYear() + '-' + String(d.getMonth()+1).padStart(2,'0') + '-' + String(d.getDate()).padStart(2,'0');
  const slug = title.toLowerCase().replace(/[^a-z0-9]+/g,'-').replace(/^-|-$/g,'').substring(0,50);
  const excerpt = body.replace(/[#>*_]/g,'').substring(0,160).trim() + '...';

  generatedFilename = dateStr + '-' + slug + '.md';
  generatedContent = `---
layout: post
title: "${title}"
era: ${era}
...
${body}`;
  ...
  document.getElementById('out-filename').textContent = generatedFilename;
  document.getElementById('out-code').value = generatedContent;
...
function copyCode() {
  const ta = document.getElementById('out-code');
  ta.select();
  document.execCommand('copy');
  alert('Copied! Now paste it into a new file in your GitHub _posts folder.');
}
